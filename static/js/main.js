...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function setLanguage(lang) {
    ...
    if (fileLabel && t["contact_form_file"])
      fileLabel.textContent = t["contact_form_file"];
  }
  ...
  function openModal(projectKey) {
    ...
    if (modalPrev)
      modalPrev.style.display = currentImages.length > 1 ? "" : "none";
    if (modalNext)
      modalNext.style.display = currentImages.length > 1 ? "" : "none";
  ...
  if (modalPrev)
    modalPrev.addEventListener("click", () => goToSlide(currentIndex - 1));
  if (modalNext)
    modalNext.addEventListener("click", () => goToSlide(currentIndex + 1));
